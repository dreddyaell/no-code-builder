...
interface LayoutBuilderProps {
  ...
  footerItems: { [key: string]: FooterItem[] };
...
export default function LayoutBuilder({
  ...
  const updateItemPosition = (id: string, x: number, y: number) => {
    ...
    } else if ((footerItems[selectedFooter] || []).some((i) => i.id === id)) {
      const updated = footerItems[selectedFooter].map((item) =>
  ...
  const saveEditChanges = () => {
    ...
    if (headerItems.some((i) => i.id === selectedItem.id)) {
      const updated = headerItems.map((item) =>
        item.id === selectedItem.id ? selectedItem : item
      );
      setHeaderItems((prev) => ({ ...prev, [selectedHeader]: updated }));
      localStorage.setItem(`headerItems-${selectedHeader}`, JSON.stringify(updated));
    } else if (bodyItems.some((i) => i.id === selectedItem.id)) {
      const updated = bodyItems.map((item) =>
        item.id === selectedItem.id ? selectedItem : item
      );
      setBodyItems(updated);
      localStorage.setItem(`bodyItems-${selectedBody}`, JSON.stringify(updated));
    } else if ((footerItems[selectedFooter] || []).some((i) => i.id === selectedItem.id)) {
      const updated = footerItems[selectedFooter].map((item) =>
        item.id === selectedItem.id ? selectedItem : item
      );
      setFooterItems((prev) => ({ ...prev, [selectedFooter]: updated }));
  ...
  const removeElement = (targetId: string) => {
    setHeaderItems((prev) => {
      const updated = (prev[selectedHeader] || []).filter((item) => item.id !== targetId);
      localStorage.setItem(`headerItems-${selectedHeader}`, JSON.stringify(updated));
    ...
    setFooterItems((prev) => {
      const updated = (prev[selectedFooter] || []).filter((item) => item.id !== targetId);
      localStorage.setItem(`footerItems-${selectedFooter}`, JSON.stringify(updated));
